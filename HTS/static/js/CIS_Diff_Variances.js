// Main form submission function for two-variance confidence interval test
async function submitForm(event) {
    ...
    try {
        ...
        // API Request - changed endpoint to confidence interval version
        const response = await fetch('/calculate_two_variances_ci', {
            method: 'POST',
        ...
        document.getElementById('resultContent').innerHTML = `
            <p><strong>Variance Ratio (s₁²/s₂²):</strong> ${result.varianceRatio}</p>
            <p><strong>${result.confidenceLevel*100}% Confidence Interval:</strong> [${result.confidenceInterval[0]}, ${result.confidenceInterval[1]}]</p>
            <p><strong>Degrees of Freedom:</strong> df₁ = ${result.df1}, df₂ = ${result.df2}</p>
            <p><strong>Critical F-values:</strong> Lower = ${result.criticalFLower}, Upper = ${result.criticalFUpper}</p>
